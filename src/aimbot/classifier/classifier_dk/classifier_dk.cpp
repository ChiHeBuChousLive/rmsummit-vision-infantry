...
Classifier_dk::Classifier_dk(char *cfg_file, char *weight_file,
    ...
    size_t srcSize = 56 * 56 * 3 * sizeof(float);
    input = (float *)malloc(srcSize);
}

int Classifier_dk::predict(const cv::Mat &src) {
    // 将图像转为yolo形式
...
void Classifier_dk::imgConvert(const cv::Mat &img, float *dst) {
    uchar *data = img.data;
    int h = img.rows;
    int w = img.cols;
    int c = img.channels();
